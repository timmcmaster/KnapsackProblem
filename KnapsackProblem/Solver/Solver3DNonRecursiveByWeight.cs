...
namespace KnapsackProblem.Solver
{
    ...
    public class Solver3DNonRecursiveByWeight : ISolver
    {
        ...
        public Solver3DNonRecursiveByWeight(Knapsack knapsack, List<Item> items)
        {
            ...
            _maxProfitItemGroup = new ItemGroup[numberOfItemsInList, numberOfWeights, numberOfItemsAllowed];
        }
        ...
        public void CalculateNonRecursive()
        {
            ...
            for (int weight = 0; weight <= _maxProfitItemGroup.GetUpperBound(1); weight++)
            {
                ...
                for (int itemsChosen = 0; itemsChosen <= _maxProfitItemGroup.GetUpperBound(2); itemsChosen++)
                {
                    // maximum value for first 0 items at any given weight and no. chosen is 0
                    _maxProfitItemGroup[0, weight, itemsChosen] = new ItemGroup();
                }
            }
            ...
            for (int i = 1; i <= _maxProfitItemGroup.GetUpperBound(0); i++)
            {
                ...
                for (int weight = 0; weight <= _maxProfitItemGroup.GetUpperBound(1); weight++)
                {
                    ...
                    for (int itemsChosen = 0; itemsChosen <= _maxProfitItemGroup.GetUpperBound(2); itemsChosen++)
                    {
                        // Calculate m[i,j,k] which is the maximum value that can be obtained at weight j after considering first i elements in list and choosing k items 
                        ...
                        if (_items[i - 1].Weight > weight)
                        {
                            ...
                            _maxProfitItemGroup[i, weight, itemsChosen] = new ItemGroup(_maxProfitItemGroup[i - 1, weight, itemsChosen]);
                        }
                        else
                        {
                            ItemGroup withoutCurrentItem =
                                _maxProfitItemGroup[i - 1, weight - _items[i - 1].Weight, itemsChosen - 1];
